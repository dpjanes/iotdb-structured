 ...
 *  Weights look like this:
   ...
   { '@type': 'schema:StructuredValue',
     'schema:value': 1,
     'schema:unitCode': 'qudt:Gram' }

 */
const total = (values, paramd) => {
    ...
    paramd.unitCode = "qudt:Gram"

    if (_.is.Nullish(values)) {
        return paramd.otherwise;
    }

    const total_weight = 0

    _.coerce.list(values)
        .forEach(value => {
            assert.ok(_.is.Dictionary(value))

            const weight = value["schema:value"]
            ...
            const unitCode = value["schema:unitCode"]
