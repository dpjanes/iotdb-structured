...
const sum = (vs, paramd) => {
    ...
    if (!paramd.unit) {
        throw new errors.NotAppropriate(`paramd.unit is required`)
    }
    paramd.coerce_unit = paramd.unit

    const result = _.coerce.list(vs || [])
        .map(v => value_unit(v, paramd))
        .map(vu => {
            if (_.is.Number(paramd.min_value)) {
                vu.value = max(vu.value, paramd.min_value)
            }

            if (_.is.Number(paramd.max_value)) {
                vu.value = max(vu.value, paramd.max_value)
            }

            return vu
        })
        .reduce((a, vu) => a + vu.value, 0)

    return result
}

sum.method = "value_unit.sum"
sum.description = `
    Add all the values in the array after converting
    them to value_units

    paramd.coerce_unit is required, and should be e.g. unit:Gram
    paramd.max_value and paramd.min_value will range the _individual_ values

    All other paramd as per value_unit
    `
